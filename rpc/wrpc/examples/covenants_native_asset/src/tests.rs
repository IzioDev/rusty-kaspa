...
use crate::covenants::{
    ...
    token_state_from_spk, try_spk_bytes, MinterState, NativeAssetOutput, TokenInputRef,
};
...
impl Harness {
    fn new() -> Self {
        ...
        let minter_outpoint = TransactionOutpoint::new(Hash::from_u64_word(10), 0);
        let covenant_id = covenant_id_for_outpoint(&minter_outpoint);
        let minter_state =
            MinterState { covenant_id, remaining_supply: TEST_TOTAL_SUPPLY, authority_spk_bytes: authority_spk_bytes.clone() };
    ...
    fn build_split_merge_tx(&self, tokens: &[TokenUtxo], outputs: &[NativeAssetOutput], auth: &SpendableUtxo) -> Transaction {
        ...
        build_token_split_merge_tx(&token_refs, outputs, auth_input, auth.entry.clone(), &self.mass_calculator)
