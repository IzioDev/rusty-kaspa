...
use kaspa_txscript::opcodes::codes::{
    Op2Drop, OpAdd, OpAuthOutputCount, OpAuthOutputIdx, OpBin2Num, OpDrop, OpDup, OpEndIf, OpEqual, OpEqualVerify,
    OpGreaterThanOrEqual, OpIf, OpInputCovenantId, OpNumEqualVerify, OpPick, OpSize, OpSub, OpSubStr, OpSwap, OpTrue, OpTxInputCount,
    OpTxInputIndex, OpTxInputSpk, OpTxInputSpkLen, OpTxInputSpkSubstr, OpTxOutputCount, OpTxOutputSpkLen, OpTxOutputSpkSubstr,
    OpVerify, OpWithin,
};
...
use crate::script_layout::{
    AMOUNT_LEN, SPK_AMOUNT_OFFSET, SPK_BYTES_MAX, SPK_BYTES_MIN, SPK_FIELD_LEN, SPK_FIELD_LEN_OFFSET, SPK_LOGIC_TAIL_OFFSET,
    SPK_SPK_FIELD_OFFSET,
};
...
impl MinterState {
    pub fn mint_next(&self, amount: u64) -> CovenantResult<Self> {
        ...
        Ok(Self { remaining_supply: remaining, authority_spk_bytes: self.authority_spk_bytes.clone() })
    }
}
...
pub fn minter_state_from_spk(spk: &ScriptPublicKey) -> CovenantResult<MinterState> {
    ...
    Ok(MinterState { remaining_supply, authority_spk_bytes })
}
...
pub fn token_state_from_spk(spk: &ScriptPublicKey) -> CovenantResult<TokenState> {
    ...
    Ok(TokenState { amount, owner_spk_bytes })
}
...
pub fn build_minter_covenant_script_knat20(state: &MinterState) -> CovenantResult<Vec<u8>> {
    ...
    let encoded_state = encode_script_state(state.remaining_supply, &state.authority_spk_bytes)?;
...
pub fn build_token_covenant_script_knat20(state: &TokenState) -> CovenantResult<Vec<u8>> {
    ...
    let encoded_state = encode_script_state(state.amount, &state.owner_spk_bytes)?;
    let token_logic = build_token_covenant_logic()?;
...
pub fn build_mint_tx(
    ...
    let token_state = TokenState { amount: mint_amount, owner_spk_bytes: recipient_spk_bytes };
    ...
    apply_covenant_info(&mut temp_outputs, covenant_id.clone(), 0);
    ...
    apply_covenant_info(&mut outputs, covenant_id.clone(), 0);
...
pub fn build_token_transfer_tx(
    ...
    build_token_split_merge_tx(&[token_input], covenant_id, &[output], auth_input, auth_entry, mass_calculator)
}
...
pub fn build_token_split_merge_tx(
    ...
    for output in outputs {
        ...
        let token_state = TokenState { amount: output.amount, owner_spk_bytes: output.recipient_spk_bytes.clone() };
        let token_script = build_token_covenant_script_knat20(&token_state)?;
        let token_spk = ScriptPublicKey::from_vec(0, token_script);
    ...
    apply_covenant_info(&mut temp_outputs, covenant_id.clone(), TOKEN_LEADER_INPUT_INDEX as u16);
    ...
    apply_covenant_info(&mut final_outputs, covenant_id.clone(), TOKEN_LEADER_INPUT_INDEX as u16);
...
fn encode_script_state(amount: u64, spk_bytes: &[u8]) -> CovenantResult<Vec<u8>> {
    ...
    sb.add_data(&amount_bytes)?.add_data(&spk_field)?;
    Ok(sb.drain())
}
...
fn build_minter_covenant_logic(token_logic: &[u8]) -> CovenantResult<Vec<u8>> {
    ...
    sb.add_i64(0)?.add_op(OpAuthOutputCount)?.add_i64(2)?.add_op(OpEqualVerify)?;
...
fn build_token_covenant_logic() -> CovenantResult<Vec<u8>> {
    ...
    sb.add_i64(TOKEN_LEADER_INPUT_INDEX)?.add_op(OpAuthOutputCount)?;
    sb.add_op(OpNumEqualVerify)?;
    ...
    for idx in 2..=MAX_SPLIT_MERGE_INPUTS_COUNT {
        ...
        push_input_covenant_id(&mut sb, TOKEN_LEADER_INPUT_INDEX)?;
        push_input_covenant_id(&mut sb, idx as i64)?;
        sb.add_op(OpEqualVerify)?;
        sb.add_op(OpEndIf)?;
    }
    ...
    for idx in 1..MAX_SPLIT_MERGE_OUTPUTS_COUNT {
        sb.add_i64(TOKEN_LEADER_INPUT_INDEX)?.add_op(OpAuthOutputCount)?.add_i64((idx + 1) as i64)?.add_op(OpGreaterThanOrEqual)?;
        sb.add_op(OpIf)?;
...
fn push_input_covenant_id(sb: &mut ScriptBuilder, idx: i64) -> Result<(), ScriptBuilderError> {
    sb.add_i64(idx)?.add_op(OpInputCovenantId)?;
    Ok(())
}
...
fn push_authorized_output_amount(sb: &mut ScriptBuilder, authorizing_input: i64, k: i64) -> Result<(), ScriptBuilderError> {
    sb.add_i64(authorizing_input)?.add_i64(k)?.add_op(OpAuthOutputIdx)?;
...
fn verify_authorized_output_at_index_script_matches_authority_script(
    ...
    sb.add_i64(k)?.add_op(OpAuthOutputIdx)?;
...
fn verify_authorized_output_at_index_script_matches_script(
    ...
    sb.add_i64(0)?.add_i64(k)?.add_op(OpAuthOutputIdx)?;
    push_output_script_from_index(sb)?;
