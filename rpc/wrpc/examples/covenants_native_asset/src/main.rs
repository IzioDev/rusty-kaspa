...
use covenants::{
    MinterState, TokenState, build_mint_tx, build_minter_covenant_script_knat20, build_token_transfer_tx, minter_state_from_spk,
    token_state_from_spk, try_spk_bytes,
};
use kaspa_addresses::Prefix;
use kaspa_bip32::{AddressType, ChildNumber, ExtendedPrivateKey, Prefix as KeyPrefix, PrivateKey, SecretKey, secp256k1::PublicKey};
...
use kaspa_consensus_core::sign::{Signed, sign_with_multiple_v2};
...
async fn create_native_asset_flow(wallet_ctx: &WalletContext) -> Result<()> {
    ...
    let seed_state = MinterState { remaining_supply: TOTAL_SUPPLY, authority_spk_bytes: authority_spk_bytes.clone() };
    let seed_script = build_minter_covenant_script_knat20(&seed_state).map_err(map_covenant_err)?;
    let seed_spk = ScriptPublicKey::from_vec(0, seed_script);

    let genesis_input = TransactionInput::new(genesis_outpoint, vec![], 0, 1);
    let temp_genesis_output = TransactionOutput::new(genesis_entry.amount, seed_spk.clone());
    ...
    let deployment_output = TransactionOutput::new(minter_value, seed_spk);
    let covenant_id = covenant_id(genesis_outpoint, std::iter::once((0, &deployment_output)));
    println!("covenant id: {}", covenant_id);

    let genesis_state = MinterState { remaining_supply: TOTAL_SUPPLY, authority_spk_bytes: authority_spk_bytes.clone() };
